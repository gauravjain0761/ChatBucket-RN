...
import { DELETE_MESSAGE_MODAL_VISIBLE } from "../../redux/actionTypes";
type Props = {};
...
const PersonalChatHeader = (props: Props) => {
  ...
  console.log(activeChat?.opponentDetails);
  return (
    <SafeAreaView>
      {selectedMessage == undefined ? (
        <View style={[styles.headerMainView, { paddingLeft: 0 }]}>
          <View style={[styles.header, { gap: 0 }]}>
            ...
            <TouchableOpacity style={styles.header}>
              <TouchableOpacity>
                <RenderUserIcon
                  url={activeChat?.opponentDetails?.image}
                  size={50}
                />
              </TouchableOpacity>
              <View>
                <Text style={{ ...commonFontStyle(600, 16, colors.white) }}>
                  {activeChat?.opponentDetails?.name}
                </Text>
                <Text style={{ ...commonFontStyle(500, 14, colors.white) }}>
                  Online
                </Text>
              </View>
            </TouchableOpacity>
        ...
        <View style={[styles.headerMainView, { paddingLeft: 0 }]}>
          ...
          <View style={[styles.header, { gap: 0 }]}>
            ...
            <TouchableOpacity
              onPress={() =>
                dispatchAction(dispatch, DELETE_MESSAGE_MODAL_VISIBLE, true)
              }
